...
import { ReactComponent as FlokiImg } from "src/assets/tokens/FLOKI.svg";
...
import { abi as FlokiBondContract } from "src/abi/bonds/FlokiBondContract.json";
...
async function getFlokiPrice() {
  try {
    let flokiPrice = await axios.get(`https://api.coingecko.com/api/v3/simple/price?ids=floki-inu&vs_currencies=usd`);
    return flokiPrice.data['floki-inu'].usd;
  } catch (e) {
    console.log("coingecko api error: ", e);
  }
}
...
export const floki = new CustomBond({
  ...
  customTreasuryBalanceFunc: async function (this: CustomBond, networkID, provider) {
    ...
    const token = this.getContractForReserve(networkID, provider);
    let flokiAmount: BigNumberish = await token.balanceOf(addresses[networkID].TREASURY_ADDRESS);
...
export const allBonds = [shib, dog_eth, floki];
